...
class CompressedQuadtree:public StructureInterface<objecttype,struc_id,object_form>{
	int current_depth=0;
	int max_depth=0;
	// Größe und Position des Quadranten
	rect<2> frame;
	//0: left_top; 1: left_bot; 2: right_top; 3: right_bot
	PArray<CompressedQuadtree*> boundarys={nullptr,nullptr,nullptr,nullptr};
	...
	bool compress_create(byte placing,const object_form& form_fit){
		...
		if(perfect_boundary!=frame){
			boundarys[placing] = new CompressedQuadtree(perfect_boundary,max_depth,real_depth);
	...
	bool insert_compressed(byte placing,const objecttype& object) {
		...
		CompressedQuadtree* vorhandener_baum=boundarys[placing];
		boundarys[placing]=nullptr;
		CompressedQuadtree* object_tree=this;
	...
	void getAllElements(PArray<objecttype>& elements){
		for(max_uint i=0;i<array.elements();i++){
	...
	CompressedQuadtree(int x,int y,int width,int height,int new_max_depth=5,int newdepth=0){
		frame.pos[0]=x;
		frame.pos[1]=y;
		frame[0]=width;
		frame[1]=height;
	...
	CompressedQuadtree(const rect<2> newframe,int new_max_depth=5,int newdepth=0){
