...
class BVHQuadtree:public StructureInterface<objecttype,struc_id,object_form>{
	int current_depth=0;
	int max_depth=0;
	// Größe und Position des Quadranten
	rect<2> frame;
	//0: left_top; 1: left_bot; 2: right_top; 3: right_bot
	PArray<BVHQuadtree*> boundarys={nullptr,nullptr,nullptr,nullptr};
	//Elemente in Quadranten
	bool compressed=false;
	max_uint current_elements=0;
	BVH<objecttype,struc_id,object_form> tree;
	...
	bool removeElementByFrame(const objecttype& object,const object_form& old_frame,int mode=0){
		if(mode!=0){
			if(!frame.contain(object->getRect())){
				mode=0;
			}
		}
		if(tree.removeByFrame(object,old_frame,mode)){
			if(mode!=0){
				if(tree.insert(object)){
					return false;
				}
	...
	size_t structureQuery(const object_form& query_rect,PArray<objecttype>& query_array,objecttype object_pointer=nullptr){
		return tree.query(query_rect,query_array,object_pointer);
	}
	...
	bool compress_create(byte placing,const object_form& form_fit){
		...
		if(perfect_boundary!=frame){
			boundarys[placing] = new BVHQuadtree(perfect_boundary,max_depth,real_depth);
	...
	bool insert_compressed(byte placing,const objecttype& object) {
		...
		BVHQuadtree* vorhandener_baum=boundarys[placing];
		boundarys[placing]=nullptr;
		BVHQuadtree* object_tree=this;
	...
	bool removeByFrame(const objecttype& object,const object_form& objectframe,int mode=0){
		...
		return removeElementByFrame(object,objectframe,mode);
	}
	...
	size_t query(const object_form& query_rect,PArray<objecttype>& query_array,objecttype object_pointer=nullptr) override {
		...
		size_t elements=0;
		if(current_elements>0){
			elements+=structureQuery(query_rect,query_array,object_pointer);
		}
		for(byte i=0;i<boundarys.elements();i++){
			if(boundarys[i]!=nullptr){
	...
	void drawStructure(object_form& pixel_frame,int bitwidth,uint8_t* pixels) override {
		...
		if(current_elements>0){
			tree.drawStructure(pixel_frame,bitwidth,pixels);
		}
	...
	BVHQuadtree(int x,int y,int width,int height,int new_max_depth=5,int newdepth=0){
		frame.pos[0]=x;
		frame.pos[1]=y;
		frame[0]=width;
		frame[1]=height;
	...
	BVHQuadtree(const rect<2> newframe,int new_max_depth=5,int newdepth=0){
